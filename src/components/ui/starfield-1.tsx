...
const Starfield = ({
  ...
  const bigBang = () => {
    if (sd.current.star.arr.length !== quantity) {
      sd.current.star.arr = new Array(quantity).fill(0).map<Star>(() => [
  ...
  const resize = () => {
    ...
    if (sd.current.cw !== sd.current.w || sd.current.ch !== sd.current.h) {
      ...
      const rz = oldZ > 0 ? sd.current.z / oldZ : 1;
      ...
      if (!sd.current.star.arr.length) {
        ...
        sd.current.star.arr = sd.current.star.arr.map((star, i) => {
          ...
          newStar[2] = Math.max(1, oldStar.arr[i][2] * rz);
  ...
  const draw = () => {
    ...
    sd.current.star.arr.forEach((star) => {
      if (
        ...
        ctx.lineWidth = (1 - sd.current.star.colorRatio * star[2]) * 2.8;
        ctx.beginPath();
