  ...
  return (
    <a
      ...
      className={[
        ...
        // Hover: gentle lift + a soft purple aura that bleeds outward from
        // the button.
    ...
    >
      {/* Spinning conic-gradient layer. Sized to 2x the button's longest side
          ...
          covers the full button when it spins.
          A faint `bg-white/12` sits under the conic so the dim half of the
          arc always falls back to a subtle static outline — the button's
          shape stays readable when the comet is on the far side, and the
          animated arc layers on top brightly. */}
      <span
        aria-hidden
      ...
      >
        <span className="absolute left-1/2 top-1/2 aspect-square w-[200%] -translate-x-1/2 -translate-y-1/2">
          <span
            // 6s linear rotation — slow and meditative. On hover the comet
            // brightens (saturated purple) and a hair more saturation hints
            // that the button is interactive without breaking the rhythm.
      ...
      {/* Inner fill: sits 1.5px inside the gradient layer to leave a thin
          ring visible. A touch lighter on hover so the button itself reads
          as "lifted" rather than just glowing. */}
      <span
        aria-hidden
