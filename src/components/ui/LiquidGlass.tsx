"use client";

import {
  useEffect,
  useId,
  useLayoutEffect,
  useRef,
  useState,
  type CSSProperties,
  type ElementType,
  type HTMLAttributes,
  type ReactNode,
  type Ref,
} from "react";

/**
 * LiquidGlass — a portable, dependency-free "liquid glass" surface.
 *
 * Wrap anything (navbars, cards, docks, buttons, modals, toolbars) to get an
 * Apple-style refractive glass panel. The look is three layers stacked on a
 * single element:
 *
 *   1. a faint, translucent tinted body
 *   2. `backdrop-filter: blur() <displacement> saturate()` — the frost plus the
 *      edge refraction that bends whatever sits/scrolls behind the panel
 *   3. a multi-stop inset/outset box-shadow that paints the bevel + rim light
 *
 * The SVG displacement (the refraction in layer 2) is a Chromium-only treat;
 * Safari and Firefox fall back to plain blur+saturate automatically — still
 * glass, just without the wobble. Everything is inline-styled, so the component
 * drops into ANY React project with zero CSS/Tailwind/build setup. Pass layout
 * via `className` or `style`; the glass visuals come from the props below.
 *
 * The displacement map and the bevel recipe are adapted from Vadik Matveev's
 * "Liquid glass" pen, re-shaped here into a size-aware, reusable component
 * (the displacement scales correctly from a tiny button to a full-width navbar).
 *
 * @example
 * <LiquidGlass className="flex items-center gap-4 px-6 py-3" radius={20}>
 *   …navbar content…
 * </LiquidGlass>
 */

// A pre-baked normal map (R→x shift, G→y shift, mid-gray 0.5 = no shift).
// Stretched to fill the panel, its bright rim gradients refract the backdrop
// along the border — the "thick glass edge" that sells the effect.
const DISPLACEMENT_MAP =
  "data:image/webp;base64,";

const clampPct = (n: number) => Math.max(0, Math.min(100, n));

/**
 * The signature bevel: four light stops (top-left rim, inner glow) and four dark
 * stops (bottom shadow, ambient depth), plus two soft outer drop shadows. The
 * `reflex` multipliers let one recipe serve light, dark, and tinted backgrounds.
 */
function bevelShadow(
  light: string,
  dark: string,
  lightReflex: number,
  shadowReflex: number,
): string {
  const L = (pct: number) =>
    `color-mix(in srgb, ${light} ${clampPct(lightReflex * pct)}%, transparent)`;
  const D = (pct: number) =>
    `color-mix(in srgb, ${dark} ${clampPct(shadowReflex * pct)}%, transparent)`;
  return [
    `inset 0 0 0 1px ${L(10)}`,
    `inset 1.8px 3px 0 -2px ${L(90)}`,
    `inset -2px -2px 0 -2px ${L(80)}`,
    `inset -3px -8px 1px -6px ${L(60)}`,
    `inset -0.3px -1px 4px 0 ${D(12)}`,
    `inset -1.5px 2.5px 0 -2px ${D(20)}`,
    `inset 0 3px 4px -2px ${D(20)}`,
    `inset 2px -6.5px 1px -4px ${D(10)}`,
    `0 1px 5px 0 ${D(10)}`,
    `0 6px 16px 0 ${D(8)}`,
  ].join(", ");
}

// useLayoutEffect warns during SSR; fall back to useEffect on the server.
const useIsoLayoutEffect =
  typeof window !== "undefined" ? useLayoutEffect : useEffect;

/**
 * Tracks an element's rendered border-box size so the displacement map can be
 * stretched to match exactly — this is what keeps the refraction looking right
 * whether the glass is a 40px button or a 1280px navbar.
 */
function useBoxSize<T extends HTMLElement>() {
  const ref = useRef<T>(null);
  const [size, setSize] = useState<{ w: number; h: number } | null>(null);
  useIsoLayoutEffect(() => {
    const el = ref.current;
    if (!el) return;
    const measure = () => setSize({ w: el.offsetWidth, h: el.offsetHeight });
    measure();
    const observer = new ResizeObserver(measure);
    observer.observe(el);
    return () => observer.disconnect();
  }, []);
  return [ref, size] as const;
}

export interface LiquidGlassProps extends HTMLAttributes<HTMLElement> {
  children?: ReactNode;
  /** Element to render. Default `"div"`. */
  as?: ElementType;
  /** Corner radius. A number is treated as px. Default `24`. */
  radius?: number | string;
  /** Backdrop blur in px. Default `8`. */
  blur?: number;
  /** Backdrop saturation in %. Default `150`. */
  saturation?: number;
  /** Rim refraction strength in px (Chromium only). `0` disables it. Default `50`. */
  displace?: number;
  /** Glass body tint. Default `"#bbbbbc"`. */
  tint?: string;
  /** Tint strength, 0–100. Default `12`. */
  tintOpacity?: number;
  /** Specular highlight color. Default `"#ffffff"`. */
  lightColor?: string;
  /** Ambient shadow color. Default `"#000000"`. */
  shadowColor?: string;
  /** Highlight intensity multiplier — lower it on dark UIs. Default `1`. */
  lightReflex?: number;
  /** Shadow intensity multiplier — raise it on dark UIs. Default `1`. */
  shadowReflex?: number;
  /** Extra outer shadow appended after the bevel, for floating panels that need
   *  to lift off the page (menus, popovers, modals). e.g. `"0 30px 80px -20px rgba(0,0,0,.9)"`. */
  shadow?: string;
}

export function LiquidGlass({
  children,
  as,
  radius = 24,
  blur = 8,
  saturation = 150,
  displace = 50,
  tint = "#bbbbbc",
  tintOpacity = 12,
  lightColor = "#ffffff",
  shadowColor = "#000000",
  lightReflex = 1,
  shadowReflex = 1,
  shadow,
  className,
  style,
  ...rest
}: LiquidGlassProps) {
  const Tag = (as ?? "div") as ElementType;
  const filterId = `liquid-glass-${useId().replace(/[^a-zA-Z0-9]/g, "")}`;
  const [ref, size] = useBoxSize<HTMLElement>();

  // The displacement filter needs real pixel dimensions, so it only switches on
  // after the first measurement (client-side). Until then — and forever in
  // Safari/Firefox via the -webkit- line — it's plain frosted glass.
  const refract = displace > 0 && size !== null;
  const fallbackFilter = `blur(${blur}px) saturate(${saturation}%)`;

  const surfaceStyle: CSSProperties = {
    position: "relative",
    borderRadius: typeof radius === "number" ? `${radius}px` : radius,
    backgroundColor: `color-mix(in srgb, ${tint} ${clampPct(tintOpacity)}%, transparent)`,
    backdropFilter: refract
      ? `blur(${blur}px) url(#${filterId}) saturate(${saturation}%)`
      : fallbackFilter,
    WebkitBackdropFilter: fallbackFilter,
    boxShadow: shadow
      ? `${bevelShadow(lightColor, shadowColor, lightReflex, shadowReflex)}, ${shadow}`
      : bevelShadow(lightColor, shadowColor, lightReflex, shadowReflex),
    transition:
      "background-color 400ms cubic-bezier(1, 0, 0.4, 1), box-shadow 400ms cubic-bezier(1, 0, 0.4, 1)",
    ...style,
  };

  return (
    <Tag ref={ref as Ref<HTMLElement>} className={className} style={surfaceStyle} {...rest}>
      {refract ? (
        // Absolutely positioned so it never becomes a flex/grid item of the
        // panel; the <filter> just needs to exist somewhere in the document.
        <svg
          aria-hidden
          focusable="false"
          width="0"
          height="0"
          style={{
            position: "absolute",
            width: 0,
            height: 0,
            overflow: "hidden",
            pointerEvents: "none",
          }}
        >
          <filter id={filterId} primitiveUnits="userSpaceOnUse">
            <feImage
              href={DISPLACEMENT_MAP}
              x="0"
              y="0"
              width={size.w}
              height={size.h}
              preserveAspectRatio="none"
              result="map"
            />
            <feGaussianBlur
              in="SourceGraphic"
              stdDeviation={Math.max(0.5, displace / 50)}
              result="blurred"
            />
            <feDisplacementMap
              in="blurred"
              in2="map"
              scale={displace}
              xChannelSelector="R"
              yChannelSelector="G"
            />
          </filter>
        </svg>
      ) : null}
      {children}
    </Tag>
  );
}

export default LiquidGlass;
